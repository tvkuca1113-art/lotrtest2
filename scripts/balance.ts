...
mkdirSync('qa-output',{recursive:true});writeFileSync('qa-output/balance.json',JSON.stringify({method:'Guaranteed boss and objective income only. All rings upgraded whenever affordable. Three equipped slots improved every third stage. All home tiers bought at unlock. No caches, enemy loot, selling, replay income, or random drops.',deficits,rows},null,2));console.log(JSON.stringify({deficits,stage5:rows[4],stage10:rows[9],stage20:rows[19],stage25:rows[24],stage30:rows[29]},null,2));if(tier!==5)process.exitCode=1;
